...
def XML_PMS2aTV(PMS_address, path, options):
    ...
    if not "PlexConnectUDID" in options:
        ...
        declareATV(options["PlexConnectUDID"], options["aTVAddress"])
    ...
    if cmd.startswith("SettingsToggle:"):
        ...
        dprint(
            __name__, 2, "ATVSettings->Toggle: {0} in template: {1}", parts[0], parts[1]
        )
        path = ""  # clear path - we don't need PMS-XML
        ...
        return XML_Error("PlexConnect", "SaveSettings!")
        ...
        # direct link, no PMS XML available
        return XML_PlayVideo_ChannelsV1(PMS_baseURL, path)

    elif cmd == "PlayTrailer":
        trailerID = options["PlexConnectTrailerID"]
        info = urllib.request.urlopen(
            "https://youtube.com/get_video_info?html5=1&video_id=" + trailerID
        ).read()
        ...
        if url == "":
            return XML_Error(
                "PlexConnect", "Youtube: ATV compatible Trailer not available"
            )
        ...
        if not "PlexConnectCredentials" in options:
            return XML_Error(
                "PlexConnect", "MyPlex Sign In called without Credentials."
            )

        parts = options["PlexConnectCredentials"].split(":", 1)
        (username, auth_token) = PlexAPI.MyPlexSignIn(parts[0], parts[1], options)
        PlexAPI.discoverPMS(
            UDID, g_param["CSettings"], g_param["IP_self"], {"MyPlex": auth_token}
        )
        ...
        PlexAPI.discoverPMS(
            UDID, g_param["CSettings"], g_param["IP_self"], {"MyPlex": ""}
        )
        ...
        if not "PlexConnectCredentials" in options:
            return XML_Error(
                "PlexConnect", "MyPlex HomeUser called without Credentials."
            )

        parts = options["PlexConnectCredentials"].split(":", 1)
        if len(parts) != 2:
            return XML_Error(
                "PlexConnect", "MyPlex HomeUser called with bad Credentials."
            )

        auth_token = g_ATVSettings.getSetting(UDID, "myplex_auth")
        (plexHome_user, plexHome_auth) = PlexAPI.MyPlexSwitchHomeUser(
            parts[0], parts[1], options, auth_token
        )
        PlexAPI.discoverPMS(
            UDID,
            g_param["CSettings"],
            g_param["IP_self"],
            {"MyPlex": auth_token, "PlexHome": plexHome_auth},
        )
        ...
        PlexAPI.discoverPMS(
            ...
            g_param["CSettings"],
            g_param["IP_self"],
            {"MyPlex": auth_token, "PlexHome": ""},
        )

        # stays at PlexHome mode
        g_ATVSettings.setSetting(UDID, "plexhome_enable", "True")
        g_ATVSettings.setSetting(UDID, "plexhome_user", "")
        g_ATVSettings.setSetting(UDID, "plexhome_auth", "")
        ...
        if (
            not PILBackgrounds.isPILinstalled()
            or not options["aTVFirmwareVersion"] >= "6.0"
        ):
            dprint(
                __name__,
                2,
                "disable fanart (PIL not installed or aTVFirmwareVersion<6.0)",
            )
    ...
    elif path.find("serviceSearch") != -1 or (
        path.find("video") != -1 and path.lower().find("search") != -1
    ):
    ...
    if cmd == "S_-_BABS" or cmd == "BABS":
        ...
        if cmd.find("Series") != -1:
            dir = "TVShow"
        elif cmd.find("Movie") != -1:
            dir = "Movie"
        elif cmd.find("Video") != -1 or cmd.find("Personal_Media") != -1:
            # Plex Video Files Scanner
            # Extended Personal Media Scanner
            dir = "HomeVideo"
        elif cmd.find("Photo") != -1:
            dir = "Photo"
        elif cmd.find("Premium_Music") != -1:
            dir = "Music"
        elif cmd.find("Music") != -1 or cmd.find("iTunes") != -1:
            dir = "Music"
        elif cmd.find("LiveTV") != -1:
            dir = "LiveTV"
        else:
        ...
        if not path == "" and not PMSroot and PMS_address:
            if PMS_address in ["all", "owned", "shared", "local", "remote"]:
                ...
                PMS = PlexAPI.getXMLFromPMS(
                    PMS_baseURL, path, options, auth_token, enableGzip
                )
...
def XML_ExpandNode(CommandCollection, elem, child, src, srcXML, text_tail):
    if text_tail == "TEXT":  # read line from text or tail
        ...
        dprint(
            __name__, 0, "XML_ExpandNode - text_tail badly specified: {0}", text_tail
        )
        return False
    ...
    while line != None:
        ...
        if hasattr(CCommandCollection, "TREE_" + cmd):  # expand tree, work COPY, CUT
            # remove cmd from text and tail
            line = line[:cmd_start] + line[cmd_end + 2 :]
            if text_tail == "TEXT":
                child.text = line
            elif text_tail == "TAIL":
                child.tail = line
            ...
            try:
                # expand any attributes in the parameter
                param = XML_ExpandLine(CommandCollection, src, srcXML, param)
                res = getattr(CommandCollection, "TREE_" + cmd)(
                    elem, child, src, srcXML, param
                )
            except:
                dprint(
                    __name__,
                    0,
                    "XML_ExpandNode - Error in cmd {0}, line {1}\n{2}",
                    cmd,
                    line,
        ...
        # check other known cmds: VAL, EVAL...
        elif hasattr(CCommandCollection, "ATTRIB_" + cmd):
            dprint(
                __name__, 2, "XML_ExpandNode - Stumbled over {0} in line {1}", cmd, line
            )
            pos = cmd_end
        else:
            dprint(
                __name__,
                0,
                "XML_ExpandNode - Found unknown cmd {0} in line {1}",
                cmd,
                line,
            )
            # mark unknown cmd in text or tail
...
def XML_ExpandLine(CommandCollection, src, srcXML, line):
    ...
    while True:
        ...
        # expand line, work VAL, EVAL...
        if hasattr(CCommandCollection, "ATTRIB_" + cmd):
            ...
            CCommandCollection, "TREE_" + cmd
        ):  # check other known cmds: COPY, CUT
            dprint(
                __name__, 2, "XML_ExpandLine - stumbled over {0} in line {1}", cmd, line
            )
            pos = cmd_end
        else:
            dprint(
                __name__,
                0,
                "XML_ExpandLine - Found unknown cmd {0} in line {1}",
                cmd,
                line,
            )
            line = line[:cmd_start] + "((UNKNOWN:" + cmd + "))" + line[cmd_end + 2 :]
...
class CCommandHelper:
    def __init__(self, options, PMSroot, PMS_address, path):
        ...
        self.PMS_baseURL = PlexAPI.getPMSProperty(
            self.ATV_udid, self.PMS_uuid, "baseURL"
        )
        self.variables = {}
    ...
    def getParam(self, src, param):
        ...
        # colon  # replace XML_template special chars
        param = param.replace("&col;", ":")
        ...
        param = param.replace(
            "7pmtimestamp", str(int(time.mktime(sevenDate.timetuple())))
        )
        param = param.replace(
            "11pmtimestamp", str(int(time.mktime(elevenDate.timetuple())))
        )
    ...
    def getKey(self, src, srcXML, param):
        ...
        while "/" in attrib and el != None:
            ...
            # internal variable in path
            if parts[0].startswith("#") and attrib[1:] in self.variables:
                ...
                el = el.find(
                    PlexAPI.getPMSProperty(self.ATV_udid, self.PMS_uuid, parts[0][1:])
                )
            else:
                el = el.find(parts[0])
            attrib = parts[1]
        ...
        # internal variable
        if attrib.startswith("#") and attrib[1:] in self.variables:
            ...
            dfltd = False
        # aTV property, http request options
        elif attrib.startswith("^") and attrib[1:] in self.options:
...
class CCommandCollection(CCommandHelper):
    ...
    def TREE_COPY_PLAYLIST(self, elem, child, src, srcXML, param):
        tag, leftover = self.getParam(src, param)
        playType, leftover, dfltd = self.getKey(
            src, srcXML, leftover
        )  # Single (default), Continuous
    ...
    def TREE_ADDXML(self, elem, child, src, srcXML, param):
        ...
        if PMS_address in ["all", "owned", "shared", "local", "remote"]:
            ...
            # IP:port or plex.tv
            auth_token = PlexAPI.getPMSProperty(
                self.ATV_udid, self.PMS_uuid, "accesstoken"
            )
            enableGzip = PlexAPI.getPMSProperty(
                self.ATV_udid, self.PMS_uuid, "enableGzip"
            )
            PMS = PlexAPI.getXMLFromPMS(
                self.PMS_baseURL, path, self.options, auth_token, enableGzip
            )

        self.PMSroot[tag] = PMS.getroot()  # store additional PMS XML
        self.path[tag] = path  # store base path

        # tree unchanged (well, source tree yes. but that doesn't count...)
        return False
    ...
    def TREE_MEDIABADGES(self, elem, child, src, srcXML, param):
        resolution, leftover, dfltd = self.getKey(
            src, srcXML, param + "/videoResolution"
        )
        ...
        if resolution not in ["720", "1080", "2k", "4k"]:
            ...
            attribs["src"] = (
                g_param["baseURL"] + "/thumbnails/MediaBadges/" + resolution + ".png"
            )
        ...
        if vCodec == "drmi" or aCodec == "drms":
            ...
            attribs["src"] = (
                g_param["baseURL"] + "/thumbnails/MediaBadges/iTunesDRM.png"
            )
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
        ...
        if container != "" and self.options["aTVFirmwareVersion"] >= "7.0":
            attribs["insertIndex"] = str(index)
            attribs["src"] = (
                g_param["baseURL"] + "/thumbnails/MediaBadges/" + container + ".png"
            )
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
            index += 1
        # Video Codec
        if vCodec != "" and self.options["aTVFirmwareVersion"] >= "7.0":
            ...
            attribs["insertIndex"] = str(index)
            attribs["src"] = (
                g_param["baseURL"] + "/thumbnails/MediaBadges/" + vCodec + ".png"
            )
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
            index += 1
        # Audio Codec
        if aCodec != "":
            attribs["insertIndex"] = str(index)
            attribs["src"] = (
                g_param["baseURL"] + "/thumbnails/MediaBadges/" + aCodec + ".png"
            )
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
            index += 1
        # Audio Channels
        if channels != "":
            attribs["insertIndex"] = str(index)
            attribs["src"] = (
                g_param["baseURL"] + "/thumbnails/MediaBadges/" + channels + ".png"
            )
            urlBadge = etree.SubElement(additionalBadges, "urlBadge", attribs)
        # Append XML
        child.append(additionalBadges)
    ...
    def ATTRIB_IMAGEURL(self, src, srcXML, param):
        ...
        # transcoder action
        transcoderAction = g_ATVSettings.getSetting(
            self.ATV_udid, "phototranscoderaction"
        )

        # image orientation
        orientation, leftover, dfltd = self.getKey(
            src, srcXML, "Media/Part/orientation"
        )
        normalOrientation = (not orientation) or orientation == "1"

        # aTV native filetypes
        parts = key.rsplit(".", 1)
        photoATVNative = parts[-1].lower() in [
            "jpg",
            "jpeg",
            "tif",
            "tiff",
        ...
        if (
            width == ""
            and transcoderAction == "Auto"
            and normalOrientation
            and photoATVNative
        ):
            # direct play
            ...
            # request transcoding
            res = PlexAPI.getTranscodeImagePath(
                key, AuthToken, self.path[srcXML], width, height
            )
    ...
    def ATTRIB_MUSICURL(self, src, srcXML, param):
        ...
        if Media != None:
            ...
            audioATVNative = (
                ...
                and int(Media.get("bitrate", "0")) <= int(maxAudioBitrateCompressed)
                or Media.get("audioCodec", "-") in ("alac", "aiff", "wav")
            )
            ...
            if audioATVNative:
                ...
                res = PlexAPI.getTranscodeAudioPath(
                    res, AuthToken, self.options, maxAudioBitrateCompressed
                )
    ...
    def ATTRIB_VIDEOURL(self, src, srcXML, param):
        ...
        if Media != None:
            # transcoder action
            transcoderAction = g_ATVSettings.getSetting(
                self.ATV_udid, "transcoderaction"
            )
            ...
            videoATVNative = (
                ...
                or Media.get("container", "-") in ("mov", "mp4")
                and Media.get("videoCodec", "-") in ("mpeg4", "h264", "drmi")
                and Media.get("audioCodec", "-") in ("aac", "drms")
            )  # remove AC3 when Dolby Digital is Off
            ...
            if DolbyDigital == "On":
                ...
                videoATVNative = (
                    ...
                    or Media.get("container", "-") in ("mov", "mp4")
                    and Media.get("videoCodec", "-") in ("mpeg4", "h264", "drmi")
                    and Media.get("audioCodec", "-") in ("aac", "ac3", "drms")
                )

            for Stream in Media.find("Part").findall("Stream"):
                if Stream.get("streamType", "") == "1" and Stream.get("codec", "-") in (
                    "mpeg4",
                    "h264",
                ):
                    if (
                        Stream.get("profile", "-") == "high 10"
                        or int(Stream.get("refFrames", "0")) > 8
                    ):
                        videoATVNative = False
                        break
                if (
                    Stream.get("scanType", "") == "interlaced"
                    or Stream.get("codec") == "mpeg2video"
                ):
                    videoATVNative = False
                    break
            ...
            if PlexAPI.getPMSProperty(self.ATV_udid, self.PMS_uuid, "local") == "1":
                qLimits = qLookup[
                    g_ATVSettings.getSetting(self.ATV_udid, "transcodequality")
                ]
            else:
                qLimits = qLookup[
                    g_ATVSettings.getSetting(self.ATV_udid, "remotebitrate")
                ]

            # subtitle renderer, subtitle selection
            subtitleRenderer = g_ATVSettings.getSetting(
                self.ATV_udid, "subtitlerenderer"
            )

            subtitleId = ""
            subtitleKey = ""
            subtitleCodec = ""
            # Todo: check 'Part' existance, deal with multi part video
            for Stream in Media.find("Part").findall("Stream"):
                if (
                    Stream.get("streamType", "") == "3"
                    and Stream.get("selected", "0") == "1"
                ):
            ...
            subtitleIOSNative = subtitleKey == "" and (
                subtitleCodec == "mov_text"
                or subtitleCodec == "ttxt"
                or subtitleCodec == "tx3g"
                or subtitleCodec == "text"
            )  # embedded
            subtitlePlexConnect = (
                subtitleKey != "" and subtitleCodec == "srt"
            )  # external
            ...
            subtitleDirectPlay = (
                subtitleId == ""
                or subtitleRenderer == "Auto"
                and ((videoATVNative and subtitleIOSNative) or subtitlePlexConnect)
                or subtitleRenderer == "iOS, PMS"
                and (videoATVNative and subtitleIOSNative)
            )
            dprint(
                __name__,
                2,
                "subtitle: IOSNative - {0}, PlexConnect - {1}, DirectPlay - {2}",
                subtitleIOSNative,
                subtitlePlexConnect,
                subtitleDirectPlay,
            )
            ...
            if (
                ...
                or transcoderAction == "Auto"
                and videoATVNative
                and int(Media.get("bitrate", "0")) < int(qLimits[2])
                and subtitleDirectPlay
            ):
                # direct play for...
                #    force direct play
                ...
                res, leftover, dfltd = self.getKey(
                    Media, srcXML, "Part[" + str(partIndex + 1) + "]/key"
                )

                # indirect... todo: select suitable resolution, today we just take first Media
                if Media.get("indirect", False):
                    # todo... check key for trailing '/' or even 'http'
                    PMS = PlexAPI.getXMLFromPMS(
                        self.PMS_baseURL, res, self.options, AuthToken
                    )
                    res, leftover, dfltd = self.getKey(
                        PMS.getroot(),
                        srcXML,
                        "Video/Media/Part[" + str(partIndex + 1) + "]/key",
                    )
                ...
                subtitle = {
                    "selected": "1" if subtitleId else "0",
    ...
    def ATTRIB_episodestring(self, src, srcXML, param):
        # getKey "defaults" if nothing found.
        parentIndex, leftover, dfltd = self.getKey(src, srcXML, param)
    ...
    def ATTRIB_unwatchedCountList(self, src, srcXML, param):
        ...
        unwatched = int(total) - int(viewed)
        if unwatched > 0:
            return self._("{0} unwatched").format(unwatched)
        else:
            return ""
    ...
    def ATTRIB_PMSCOUNT(self, src, srcXML, param):
        # -1: correct for plex.tv
        return str(PlexAPI.getPMSCount(self.ATV_udid) - 1)
    ...
    def ATTRIB_BACKGROUNDURL(self, src, srcXML, param):
        ...
        res = g_param["baseURL"]  # base address to PlexConnect
        res = res + PILBackgrounds.generate(
            self.PMS_uuid,
            key,
            auth_token,
            self.options["aTVScreenResolution"],
            g_ATVSettings.getSetting(self.ATV_udid, "fanart_blur"),
            g_param["CSettings"],
        )
...
if __name__ == "__main__":
    ...
    PMS_address = "PMS_IP"
    CommandCollection = CCommandCollection(
        options, PMSroot, PMS_address, "/library/sections"
    )
    XML_ExpandTree(CommandCollection, aTVroot, PMSroot, "main")
    XML_ExpandAllAttrib(CommandCollection, aTVroot, PMSroot, "main")
    del CommandCollection
